...
parser.add_argument('corpus_path', help='path to the corpus to translate')
parser.add_argument('output_path', help='where to save the translations')
...
with {args.decoding_strategy} decoding strategy.")
        ...
        for _ in range(args.n):
            while True:
                if args.decoding_strategy == 'ancestral_sampling':
                    hyp = to_sentence(ancestral_sampling(model, src, max_output_length), model)
                if args.decoding_strategy == 'top_k':
                    if not args.k:
                        print('\nNo value was given for k.\nPlease see get_nbest_hypotheses.py -h for help.')
                        exit()
                    hyp = to_sentence(top_k_sampling(model, src, max_output_length, args.k), model)
